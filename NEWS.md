...
* Adds support for custom Personal Data Servers (PDS). (#51)
  * `bs_auth()` gains a `host` parameter.
...
* Fixes a bug where `bs_delete_repost()` was not targeting the correct record type. (#53)
...
* Adds internal request helpers to reduce code duplication for authentication, setting a User-Agent, using a retry policy, and handling error messages. (#58)
